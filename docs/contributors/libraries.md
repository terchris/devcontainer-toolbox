...
### Example (in service script)

```bash
source "${SCRIPT_DIR}/lib/service-auto-enable.sh"

# After successful start
auto_enable_service  # Reads SCRIPT_ID from script metadata
```
...
if [[ -n "$SCRIPT_PREREQUISITES" ]]; then
    if ! check_prerequisite_configs "$SCRIPT_PREREQUISITES" "$ADDITIONS_DIR"; then
        echo "Missing prerequisites:"
        show_missing_prerequisites "$SCRIPT_PREREQUISITES" "$ADDITIONS_DIR"
        exit 1
    fi
fi
```
...
Terminal display helpers (colors, spinners, progress bars).
...
Framework for multi-command scripts (used by service-*.sh).
...
- [Creating Install Scripts](creating-install-scripts.md)
- [Creating Service Scripts](creating-service-scripts.md)
